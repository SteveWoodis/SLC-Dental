...
4. Create a router and add ```/login```, ```/projects``` and ```/projects/:threadId``` as the URLS
...
5. Inject ```$location``` into ```LoginCtrl``` and use it to forward the user to the ```projects``` route after login (which is /projects as the URL, hint, look up how to use $location to redirect to a different URL). Here's an example of how to do that. Change the code to work with your app. 
```
...
6. Create a ```projects.html``` view and a ```projectsCtrl``` controller in the appropriate folder. Add the new view and
controller to the ```projects``` route in ```app.js```.
7. Test your login and make sure that it forwards you to the stubbed projects view.
...
1. Create a projectservice and put it the appropriate folder.
2. Create methods named ```getprojects``` and ```getThread``` to generate AngularFire references to all projects and any
...
```
angular.module('rtfmApp')
  .service('projectservice', function projectservice(EnvironmentService, $firebase) {
    ...
    return {
      ...
      getThread: function (threadId) {
        return $firebase(new Firebase(firebaseUrl + '/projects/' + threadId));
      }
    }
  });
```

3. Inject the ```projectsRef``` into the ```projectsCtrl``` using a ```resolve``` attribute in your router.
...
4. Open up your ```projectsCtrl``` located in ```projects.js```. Add ```projectsRef``` to its injections and bind
```projects.$asArray()``` to scope.
...
angular.module('rtfmApp')
  .controller('projectsCtrl', function ($scope, projectsRef) {
...
### Step 5: Set up projects view

1. Let's set up ```projects.html``` with a list of projects, an input and a button to create a new thread, and links to
...
<div>
    ...
    <ul>
        <li ng-repeat="thread in projects">
...
2. You'll need to create a function in your ```projectsCtrl``` named ```createThread```. This function must be attached
to ```$scope``` and should accept a username and a thread title as arguments. It will then use the AngularFire "array"
```$add``` function to add the new thread to the ```projects``` array. Once you get this working, you'll be able to
add projects in your view and watch them automatically add themselves to the projects list.
...
that uses ```$route.current.params.threadId``` and ```projectservice.getThread()``` to inject each thread's AngularFire ref into
your new ```ThreadCtrl```.
...
to it seamlessly. This will require creating a new ```getComments``` method in ```projectservice``` and injecting this
new ```commentsRef``` into ```ThreadCtrl``` using a ```resolve``` in your ```thread``` route.

This may seem like a lot of steps, but you've already gone through these steps twice with ```projectsRef``` and
  ...
  .service('projectservice', function projectservice(EnvironmentService, $firebase) {
    ...
    return {
      ...
      getComments: function (threadId) {
        return $firebase(new Firebase(firebaseUrl + '/projects/' + threadId + '/comments'));
      }
    }
  });
